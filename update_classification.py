...
def update_classify(sk_id, filename, get_last_updated_date):
    ...
    try:
        ...
        print("Update_Classification function starts ...")
        ...
        if filename:
            ...
            final_vendor_df = final_vendor_df[final_vendor_df['Date'] >= get_last_updated_date]
            print(final_vendor_df.shape[0])
            ...
            final_client_df = final_client_df[final_client_df['Date'] >= get_last_updated_date]
            print(final_client_df.shape[0])

            vendor_filename = full_path + "final_vendors_" + sk_id + ".csv"

            client_filename = full_path + "final_clients_" + sk_id + ".csv"

            if os.path.exists(vendor_filename):
                old_vendor_df = pd.read_csv(vendor_filename)
                merge_vendor_frames = [old_vendor_df, final_vendor_df]
                final_vendor_dataframe = pd.concat(merge_vendor_frames, axis=0)
                print(final_vendor_dataframe.shape[0])
                final_vendor_dataframe.to_csv(vendor_filename, index=False)
            else:
                final_vendor_df.to_csv(vendor_filename, index=False)

            if os.path.exists(client_filename):
                old_client_df = pd.read_csv(client_filename)
                merge_client_frames = [old_client_df, final_client_df]
                final_client_dataframe = pd.concat(merge_client_frames, axis=0)
                print(final_client_dataframe.shape[0])
                final_client_dataframe.to_csv(client_filename, index=False)
            else:
                final_client_df.to_csv(client_filename, index=False)

            print('Update_Classification function is done... ')
